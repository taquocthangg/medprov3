...
export const getDoanhThuHospital = ({  id_benhvien }) => new Promise(async (resolve, reject) => {
    try {
        ...
        const priceHospital=response?.rows?.map((item)=>parseFloat(item?.price))
        const total_Hospital=priceHospital?.reduce((total_Hospital,item)=>total_Hospital+item)
    
        console.log(priceHospital)
        
        // const priceHospital=response?.MedicalHistory?.ro
        resolve({
            err: response ? 0 : -1,
            mess:response?"Lấy thông tin doanh thu thành công":"Lấy thông tin doanh thu thành công",
            // MedicalHistory:response,
...
export const getSearchDoctor = ({ id_benhvien, name, sdt }) => new Promise(async (resolve, reject) => {
    try {
        ...
        resolve({
            err: response ? 0 : -1,
            mess:response?"Lấy thông tin bác sĩ thành công":"Lấy thông tin bác sĩ thất bại",
            User:response
        })
    } catch (e) {
        reject(e);
...
export const getAllLichSuKhamStatus = ({ id_doctor, status }) => new Promise(async (resolve, reject) => {
    ...
    try {
        ...
        // Kiểm tra nếu status không phải là null thì bổ sung vào điều kiện where
        if (status !== "") {
