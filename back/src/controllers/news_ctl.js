...
export const addComents = async (req, res,) => {
    try {
        ...
        if (!newsId, !userID, !content) return res.status(400).json({
            err: 1,
            mess: "Điền đầy đủ thông tinnhư tiêu đề và ảnh mô tả"
        })
        const response = await services.addComments({ newsId, userID, content })
