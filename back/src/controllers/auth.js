...
export const getAllLichSuKhamStatus = async (req, res) => {
    try {
        const { id_doctor } = req.params;
        const { status } = req.body;
        const response = await services.getAllLichSuKhamStatus({ id_doctor, status })
        return res.status(200).json(response)
    } catch (error) {
        res.status(500).json({ error: 'Lỗi trong quá trình lấy bệnh án' });
    }
...
export const getDoanhThuHospital = async (req, res) => {
    try {
        const { id_benhvien } = req.params;
        const response = await services.getDoanhThuHospital({ id_benhvien })
        return res.status(200).json(response)
    } catch (error) {
        res.status(500).json({ error: 'Lỗi trong quá trình lấy bệnh án' });
    }
